...
SCRIPT_NAME = 'RTD Testing Development Tools'
SCRIPT_VERS = '0.01'
SCRIPT_COPYRIGHT = '2025'
SCRIPT_AUTHOR = 'Bob Swift'

PACKAGE_NAME = 'rtd_testing'
PACKAGE_TITLE = 'ReadTheDocs Testing'

OUTPUT_DIR = '_docs'
BASE_FILE_NAME = conf.base_filename if hasattr(conf, 'base_filename') and conf.base_filename else 'musicbrainz'
FILE_NAME_ROOT = 'RTD_Test'
...
Commands:
   ...
   languages        Display list of supported languages
...
MD_RULES = {
    ...
    'MD002': 'ERROR',       # First heading of the document should be a top-level heading.
    'MD003': 'ERROR',       # Heading style should be consistent throughout the document.
    'MD004': 'WARNING',     # Inconsistent Unordered List Start style.
    'MD005': 'ERROR',       # Inconsistent indentation for list items at the same level.
    'MD006': 'INFO',        # Consider starting bulleted lists at the beginning of the line.
    'MD007': 'WARNING',     # Unordered list indentation.
    ...
    'MD013': 'WARNING',     # Line length.
    'MD014': 'ERROR',       # Dollar signs used before commands without showing output.
    'MD018': 'WARNING',     # No space present after the hash character on a possible Atx Heading.
    'MD019': 'ERROR',       # Multiple spaces are present after hash character on Atx Heading.
    'MD020': 'WARNING',     # No space present inside of the hashes on a possible Atx Closed Heading.
    'MD021': 'ERROR',       # Multiple spaces are present inside hash characters on Atx Closed Heading.
    'MD022': 'INFO',        # Headings should be surrounded by blank lines.
    'MD023': 'ERROR',       # Headings must start at the beginning of the line.
    'MD024': 'WARNING',     # Multiple headings cannot contain the same content.
    'MD025': 'ERROR',       # Multiple top-level headings in the same document.
    'MD026': 'WARNING',     # Trailing punctuation present in heading text.
    'MD027': 'WARNING',     # Multiple spaces after blockquote symbol.
    'MD028': 'WARNING',     # Blank line inside blockquote.
    'MD029': 'WARNING',     # Ordered list item prefix.
    'MD030': 'WARNING',     # Spaces after list markers.
    'MD031': 'ERROR',       # Fenced code blocks should be surrounded by blank lines.
    'MD032': 'WARNING',     # List blocks should be surrounded by blank lines.
    'MD033': 'INFO',        # Inline HTML.
    'MD034': 'ERROR',       # Bare URL used.
    'MD035': 'WARNING',     # Horizontal rule style.
    'MD036': 'WARNING',     # Emphasis possibly used instead of a heading element.
    'MD037': 'WARNING',     # Spaces inside emphasis markers.
    'MD038': 'ERROR',       # Spaces inside code span elements.
    'MD039': 'ERROR',       # Spaces inside link text.
    'MD040': 'WARNING',     # Fenced code blocks should have a language specified.
    'MD041': 'WARNING',     # First line in file should be a top-level heading.
    'MD042': 'ERROR',       # Empty link.
    'MD043': 'WARNING',     # Required heading structure.
    'MD044': 'WARNING',     # Proper names should have the correct capitalization.
    'MD045': 'WARNING',     # Images should have alternate text (alt text).
    'MD046': 'WARNING',     # Code block style.
    'MD047': 'WARNING',     # Each file should end with a single newline character.
    'MD048': 'WARNING',     # Code fence style should be consistent throughout the document.
}
...
class ErrorLint():   # pylint: disable=too-few-public-methods
    """Special Lint error for unhandled exceptions.
    """
    def __init__(self, err_type='INFO', err_line=0, err_message='', err_col=0):
        self.type = err_type
        self.line = err_line
        self.column = err_col
...
class LintMD():
    ...
    def process_error(self, err: ErrorLint):
        ...
        """
        print(f'\n   [{err.type}] L{err.line},C{err.column}: {err.message}', end='', flush=True)
        if err.type == 'WARNING':
    ...
    def check_file(self, file_name, ignore_info=True):  # pylint: disable=too-many-branches
        ...
        if not os.path.isfile(file_name):
            self.process_error(ErrorLint('ERROR', 0, 'File not found'))
            return

        try:
            # scan_result = self.linter.log_info_and_above().scan_path(file_name)
            scan_result = self.linter.log_critical_and_above().scan_path(file_name)
        except PyMarkdownApiException as ex:
            print(f"API Exception: {ex}", file=sys.stderr)
            self.process_error(ErrorLint('UNTESTED', 0, ex.reason))
            return
        except UnicodeDecodeError:
            self.process_error(ErrorLint('UNTESTED', 0, 'Unable to check because of unmapped unicode characters.'))
            return
        except IOError as ex:
            self.process_error(ErrorLint('ERROR', 0, f'Error reading file. ({ex})'))
            return

        errs = False
        for err in scan_result.scan_failures:
            err: PyMarkdownScanFailure
            rule_id = str(err.rule_id).upper()
            err_type = MD_RULES[rule_id] if rule_id in MD_RULES else 'ERROR'
            if err_type == 'INFO' and ignore_info:
                continue
            self.process_error(ErrorLint(err_type, err.line_number, f"{err.rule_description} ({err.extra_error_information})", err.column_number))
            errs = True

        for err in scan_result.pragma_errors:
            err: PyMarkdownPragmaError
            self.process_error(ErrorLint('ERROR', err.line_number, err.pragma_error))
            errs = True

        # print('\n\n' if errs else f"\r{' ' * 79}\r", end='', flush=True)
        print('\n' if errs else "  [Okay]")
    ...
    def check(self, root_dir, ignore_info=False, fail_on_warnings=False):
        ...
        for dir_name, subdir_list, file_list in os.walk(root_dir):  # pylint: disable=unused-variable
            for fname in file_list:
                if not str(fname).lower().endswith('.md'):
                    continue
                fpath = os.path.join(dir_name, fname)
                if fpath in MD_IGNORE_FILES:
                    continue
                self.check_file(fpath, ignore_info)

        text = (
            f'Checked {self.checked_count:,} file{"" if self.checked_count == 1 else "s"}.  '
...
class LintRST():
    ...
    def check_file(self, file_name, ignore_info=True):  # pylint: disable=too-many-branches
        ...
        try:
            ...
            err = ErrorLint('UNTESTED', 0, 'Unable to check because of unmapped unicode characters.\n')
            self.process_error(err)
        except IOError as ex:
            self.process_error(ErrorLint('ERROR', 0, f'Error reading file. ({ex})'))
...
def parse_command_line():
    ...
    parser01.add_argument(
        ...
        nargs='+',
        type=str,
        choices=['rst', 'md', 'sphinx', 'po', 'flake8', 'pylint', 'isort', 'python'],
        help="rst = lint check the reST files, "
             "md = lint check the markdown files "
    ...
    parser01.add_argument(
        ...
        action='store_true',
        dest='ignore_info',
        help="ignore info findings"
    )

    parser01.add_argument(
        '-w', '--no-fail-warnings',
        action='store_true',
        dest='no_fail_warnings',
    ...
    parser02.add_argument(
        ...
        action='store',
        nargs='+',
        choices=['pot', 'po', 'mo', 'html', 'pdf', 'epub', 'all', 'requirements'],
        help="pot = build translation template files, "
             "po = build translation files, "
             "mo = compile translation files, "
             "html = build html files, "
             "pdf = build pdf file, "
             "epub = build epub file, "
             "all = build all project files, "
             "requirements = build requirements.txt file"
    )
...
def run_lint_rst(root_dir, ignore_info=False, fail_on_warnings=False):
...
def run_lint_md(root_dir, ignore_info=False, fail_on_warnings=False):
    """Check the MD files in the specified directory and subdirectories.
    ...
    Keyword Arguments:
        ...
        fail_on_warnings {bool} -- Determines whether warnings will cause the check to return a failed status (default: {False})
    """
    print(f'\nLint checking all MD files starting in the "{root_dir}" directory.\n')
    linter = LintMD()
    err = linter.check(root_dir, ignore_info, fail_on_warnings)
    exit_code = 1 if err > 0 else 0
    if exit_code:
...
def run_pylint():
    ...
    for filename in python_files_to_check():
        ...
        result = subprocess.run(command, shell=True, check=False, capture_output=True, timeout=PROCESS_TIMEOUT)
...
def remove_dir(dir_path, dir_name='temporary build'):
    ...
    if os.path.exists(dir_path):
        if os.path.isdir(dir_path):
            clean_directory(dir_path, dir_name, create_dir=False)
            err = None
            tries = 5
            while tries > 0:
                tries -= 1
...
def run_command(command, timeout=PROCESS_TIMEOUT):
    ...
    try:
        exit_code = subprocess.run(command, shell=True, check=True, capture_output=False, timeout=timeout).returncode
...
def do_build(target=None, language='', clean=False):
    ...
    run_command(command, timeout=SPHINX_.BUILD_TIMEOUT)
...
def build_html(language=''):
    ...
    """
    output_dir = os.path.join(OUTPUT_DIR, 'html', language)
    html_dir = os.path.join(SPHINX_.BUILD_DIR, SPHINX_.BUILD_TARGETS['html']['dir'])
    # clean_directory(output_dir, 'html language')
    try:
        remove_dir(output_dir, 'html language')
...
def build_pdf(language=''):
    ...
    try:
        ...
        target_file = os.path.join(OUTPUT_DIR, f'{FILE_NAME_ROOT}_{CURRENT_VERSION}_[{language}].pdf')
    except (OSError) as ex:
...
def build_epub(language=''):
    ...
    target_file = os.path.join(OUTPUT_DIR, f'{FILE_NAME_ROOT}_{CURRENT_VERSION}_[{language}].epub')
    print(f'Copying output to: {target_file}\n')
    try:
...
def check_language(language, supported_only=False):
    ...
    if language and isinstance(language, str):
        if RE_TEST_LANGUAGE.match(language):
            ...
            if (not supported_only) or language in get_languages():
                return True
    return False
...
def list_languages():
    ...
    for lang_id in get_languages():
        lang_name = LANGUAGE_NAMES[lang_id[:2]] if lang_id[:2] in LANGUAGE_NAMES else 'Unknown Language'
...
def update_po(language, filegroup, filename):
    ...
    try:
        results = subprocess.run(command, shell=True, check=True, capture_output=True, timeout=PROCESS_TIMEOUT)
...
def update_po_files_for_language(language):
    ...
    try:
        use_msgmerge = not subprocess.run(command, shell=True, check=True, capture_output=True, timeout=PROCESS_TIMEOUT).returncode
...
def update_mo(language, fileroot):
    """Updates the specified po file.
    ...
    Args:
        ...
        filename (str): File name (without extension) to update
    """
    print(f'{language}: {fileroot}: ', end='', flush=True)
    ...
    command = f"msgfmt -v {po_file} -o {mo_file}"
    try:
        results = subprocess.run(command, shell=True, check=True, capture_output=True, timeout=PROCESS_TIMEOUT)
        exit_code = results.returncode
        if results.stdout.strip():
...
def update_mo_files_for_language(language):
    """Update the po files for the specified language.
    ...
    # Check if `msgmerge` is available
    command = "msgfmt --version"
    try:
        use_msgfmt = not subprocess.run(command, shell=True, check=True, capture_output=True, timeout=PROCESS_TIMEOUT).returncode
    except (SubprocessError, FileNotFoundError, OSError):
        use_msgfmt = False

    if not use_msgfmt:
        print("Unable to find 'msgfmt' program.")
        exit_with_code(1)

    root_dir = os.path.join(SPHINX_.LOCALE_DIR, short_lang, 'LC_MESSAGES')
    ...
    for dirpath, _dirnames, filenames in os.walk(root_dir):
        for filename in filenames:
            if not filename.endswith('.po'):
                continue
            fileroot = filename[:-3]
            update_mo(language, os.path.join(dirpath, fileroot))
...
def update_all_mo_files():
    """Helper function to build all language po files.
    """
    print('\nUpdating MO files for other languages.')
    for language in sorted(get_languages()):
        update_mo_files_for_language(language)


def build_requirements():
    print("\nBuilding 'requirements.txt' file.")
...
def main():
    ...
    if 'language' in vars(args):
        ...
        if args.language == 'all':
            process_languages.extend(get_languages())
        elif args.language == DEFAULT_LANGUAGE:
            pass
        elif check_language(args.language, supported_only=True):
            process_languages = [args.language]
        else:
            print(f'\nInvalid language selected: {args.language}')
            if not ('build_target' in vars(args) and (args.build_target in {'po', 'mo'})):
                print('\nPlease select from:')
                list_languages()
            exit_with_code(1)

    if 'show_languages' in vars(args):
        print('\nSupported languages are:')
        list_languages()
        sys.exit(0)

    # print(f"Process Languages: {process_languages}")
    # exit_with_code(1)

    if 'build_targets' in vars(args):
        ...
        for target in args.test_targets:
            if target == 'rst':
                run_lint_rst(SPHINX_.SOURCE_DIR, ignore_info=args.ignore_info, fail_on_warnings=not args.no_fail_warnings)

            elif target == 'md':
                run_lint_md(SPHINX_.SOURCE_DIR, ignore_info=args.ignore_info, fail_on_warnings=not args.no_fail_warnings)
